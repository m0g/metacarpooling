...
xajax.tools.xml.processFragment = function (xmlNode, seq, oRet, oRequest) {
  ...
  while (xmlNode) {
    ...
    } else if ('debugmsg' == xmlNode.nodeName) {} else
      throw {
        code: 10004,
        data: xmlNode.nodeName
    }
    ++seq;
...
xajax.js.includeScript = function (command) {
  ...
  else objScript.type = command.type; if ('undefined' != typeof command.type) objScript.setAttribute('id', command.elm_id);
...
if ('undefined' == typeof JSON) xajax.js.includeScript({
    data: xajax.config.JavaScriptURI + 'xajax_js/JSON.js'
  });;
var bmf = {
  load: function () {
    oncomplete = function () {};
    ...
    var args = [];
    for (i = 1, x = arguments.length; i < x; i++) args[i - 1] = arguments[i];
    return xajax.request({
      xjxcls: 'myapp_page_ajax',
      xjxmthd: arguments[0]
    }, {
      parameters: args,
  ...
  tab: {
    load: function (tab, entry, args, load) {
      if ($('#' + tab + '-' + entry).html() == '' || load == true) {
        $('ul#' + tab + ' li[rel=' + entry + ']').addClass('loading');
        bmf.load(args, function () {
          $('ul#' + tab + ' li[rel=' + entry + ']').removeClass('loading');
          bmf.tab.change(tab, entry);
        });
      } else this.change(tab, entry);
    },
    change: function (tab, entry, title) {
      $('ul#' + tab).children().each(function () {
        var e = $(this);
        if (e.attr('rel') == entry) {
          e.show().addClass('act');
          if (title) e.children().html(title);
          $('#' + tab + '-' + entry).show();
        } else {
          e.removeClass('act');
          $('#' + tab + '-' + e.attr('rel')).hide();
    ...
    hide: function (tab, entry) {
      $('ul#' + tab + ' li[rel="' + entry + '"]').hide();
      $('div#' + tab + '-' + entry).hide();
    }
  },
  overlay: {
    open: function () {
      ...
      $(document).bind('keyup', function (e) {
        if (e.keyCode == 27) bmf.overlay.close();
      });
    },
    close: function () {
      $('.overlay').remove();
    ...
    center: function (e) {
      $(e).show().css({
        left: Math.ceil($(window).width() / 2 - $(e).innerWidth() / 2 + $(window).scrollLeft()) + 'px',
        top: Math.ceil($(window).height() / 2 - $(e).innerHeight() / 2 + $(window).scrollTop()) + 'px'
      });
    }
  },
  map: {
    show: function (place) {
      bmf.overlay.open();
      $('body').append('<div id="google_map" class="overlay"><div id="_google_map"></div><a href="javascript:;" onclick="bmf.overlay.close()" class="close">close</a></div>');
      $('#google_map').css({
        width: Math.ceil($(window).width() - 50) + 'px',
        height: Math.ceil($(window).height() - 50) + 'px'
      });
      ...
      google_initialize(3, '_google_map', place, 13);
    },
    setpoint: {
      init: function () {
        this.geocoder = new google.maps.Geocoder();
        var latLng = new google.maps.LatLng($('#form_lat').attr('value'), $('#form_lon').attr('value'));
        ...
        this.get_address(latLng);
        google.maps.event.addListener(this.marker, 'dragstart', function () {
          $('#address').html('Dragging...');
        });
        google.maps.event.addListener(this.marker, 'dragend', function () {
          bmf.map.setpoint.updateMarkerPosition(bmf.map.setpoint.marker.getPosition());
          bmf.map.setpoint.get_address(bmf.map.setpoint.marker.getPosition());
        });
      },
      updateMarkerPosition: function (latLng) {
        $('#form_lat').attr('value', latLng.lat());
        $('#form_lon').attr('value', latLng.lng());
      },
      set_address: function (address) {
        this.geocoder.geocode({
          ...
          if (results[0].geometry.location) {
            bmf.map.setpoint.marker.setPosition(results[0].geometry.location);
            bmf.map.setpoint.updateMarkerPosition(results[0].geometry.location);
            bmf.map.setpoint.map.setCenter(results[0].geometry.location);
            bmf.map.setpoint.get_address(results[0].geometry.location);
          }
        });
      },
      get_address: function (pos) {
        this.geocoder.geocode({
          latLng: pos
        }, function (responses) {
          if (responses && responses.length > 0) {
            ...
            $('#address').html('Cannot determine address at this location.');
          }
        });
      }
    }
  ...
  popup: function (url, width, height) {
    ...
    var popup = window.open('about:blank', '', 'scrollbars=yes,left=' + left + ',top=' + top + ',width=' + width + ',height=' + height);
    with(popup.document) {
      open();
      write('<html><head><meta http-equiv="refresh" content="0; url=' + url + '" /></head><body></body></html>');
      close();
    }
    return false;
  },
  ...
  filter: function (type, id) {
    $('#' + id).addClass('act');
    bmf.load(type + '_filter', id, xajax.getFormValues('filter'));
  },
  form: {
    edit: {
      step: {
        check: function (e) {
          ...
          if (e.val() != '') {
            if (e.parent().next().length == 0) {
              e.parent().after('<li class="clear">' + e.parent().html() + '</li>');
            }
          }
        },
    ...
    focus: function (e) {
      $(e).addClass('act').one('blur', function () {
        $(this).removeClass('act').removeClass('error');
      });
    },
    check: {
      number: function (e) {
        e = $(e);
        if (isNaN(parseFloat(e.val()))) e.val('');
      }
    },
    round: function (e, number) {
      e = $(e);
      if (e.val() != 0) {
        val = parseFloat(e.val().replace(',', '.')).toFixed(number);
        if (val > 0) e.val(val.toString().replace('.', ','));
        else e.val('');
      } else e.val('');
    }
  },
...
function google_initialize(art, id, address, zoom, reset) {
  if (!bmf.map2 || art == 3) {
    if (typeof (address) == 'string') {
      ...
      geocoder.geocode({
        ...
        if (status == google.maps.GeocoderStatus.OK) {
          init_map(art, id, results[0].geometry.location, zoom);
        } else alert('Geocode was not successful for the following reason: ' + status);
      });
    } else init_map(art, id, new google.maps.LatLng(address[1], address[0]), zoom);
  }
}

function init_map(art, id, latlng, zoom) {
  bmf.map2 = new google.maps.Map(document.getElementById(id), myOptions = {
    zoom: zoom,
    center: latlng,
    mapTypeId: google.maps.MapTypeId.ROADMAP
  });
  ...
  for (var i = 0; i < result.length; i++) google_marker(result[i], art);
  google.maps.event.trigger(bmf.map2, 'resize');
}

function google_marker(entry, art) {
  var marker = new google.maps.Marker({
    position: new google.maps.LatLng(entry[1], entry[2]),
    map: bmf.map2,
    icon: (art != 1) ? '/images/marker2_' + entry[3] + '.gif' : '/images/marker_' + entry[3] + '.gif',
    title: entry[0],
    zIndex: entry[3]
  });
  google.maps.event.addListener(marker, 'click', function () {
    window.location.href = '/redirect/object/' + marker.title;
  });
};

function navsize() {
  $('#mon').html('&nbsp;' + ($(window).width() + 15) + '&nbsp;');
}
...
addEvent(window, "load", navsize);
addEvent(window, "resize", navsize);
addEvent(window, "orientationchange", navsize);;
...
var _hiddenField;
var _active = false;
InstallAC = function (hfl, fld) {
  if (fld != _inputField) {
    _hiddenField = hfl;
    _inputField = fld;
    _w = 'query';
    _url = 'http://www.bessermitfahren.de/search.php';
    X = false;
    Y = true;
    Ba = true;
    installACPartTwo();
    da = false;
    _oldInputFieldValue = '';
    _inputField.value = _inputField.value.split(',')[0];
    if (document.getElementById('completeDiv')) document.getElementById('completeDiv').innerHTML = '';
    _active = true;
  }
}
...
function resizeHandler() {
  if (_w == 'url') setInputFieldSize();
  setCompleteDivSize()
}

function setCompleteDivSize() {
  if (_completeDiv && _inputField.offsetHeight) {
...
function calculateWidth() {
  if (navigator && navigator.userAgent.toLowerCase().indexOf("msie") == -1) return _inputField.offsetWidth - ca * 2
  else return _inputField.offsetWidth
}

function installACPartTwo() {
  if (getXMLHTTP()) _hasXMLHTTP = true
  else _hasXMLHTTP = false
  _completeSearchString = '/complete/search';
  if (!_hasXMLHTTP) setCookie('qu', '', 0, _completeSearchString, null, null)
  ...
  if (!$('#completeDiv').length) {
    _completeDiv = document.createElement('DIV');
    _completeDiv.id = 'completeDiv';
  } else {
    _completeDiv = document.getElementById('completeDiv');
  }
  ca = 1;
  Aa = 1;
  ...
  _completeDiv.style.zIndex = '1001';
...
function onBlurHandler() {
  if (_inputField.value == '') _hiddenField.val('');
  hideCompleteDiv();
  _inputField = '';
  _active = false;
}
  ...
  if (X && _eventKeycode != 0) {
    ...
    if (_eventKeycode == 13 || _eventKeycode == 3) {
      ...
      if (d.createTextRange) {
        ...
        t.select()
      } else if (d.setSelectionRange) d.setSelectionRange(d.value.length, d.value.length);
    } else {
      if (_inputField.value != V) selectEntry(V);
    }
  }
  _currentInputFieldValue = V;
  if (handleCursorUpDownEnter(_eventKeycode) && _eventKeycode != 0) Pa(_completeDiv, valueOfCAutoComplete);
}
newwin = function () {
...
var Cb = function () {
  ...
  _hiddenField.val(valueOfDAutoComplete(this));
...
var ec = function () {
  ...
  function highlightNewValue(C) {
    ...
    showCompleteDiv();
    if (C >= _completeDivRows) C = _completeDivRows - 1;
    ...
    _hiddenField.val(valueOfDAutoComplete(_highlightedSuggestionDiv));
  }
  ...
  function Pa(localCompleteDiv, ib) {
    ...
    if (_currentInputFieldValue == '' || O == 0) hideCompleteDiv();
    ...
    if (!ab && _highlightedSuggestionDiv) {
      ...
      if (z != localInputField.value) {
        if (localInputField.value != _currentInputFieldValue) return;
        if (X) {
          if (localInputField.createTextRange || localInputField.setSelectionRange) selectEntry(z);
          if (localInputField.createTextRange) {
            var t = localInputField.createTextRange();
            t.moveStart("character", Da.length);
            t.select()
          } else if (localInputField.setSelectionRange) localInputField.setSelectionRange(Da.length, localInputField.value.length);
        }
      }
    } else {
      _highlightedSuggestionIndex = -1;
  ...
  function lb(n) {
    ...
    if (n.createTextRange) {
      ...
      N = fa.text.length
    } else if (n.setSelectionRange) N = n.selectionEnd - n.selectionStart;
    return N;
  }
  ...
  function bb(n) {
    ...
    } else if (n.setSelectionRange) v = n.selectionStart;
    else v = -1;
    return v
  }
  ...
  function setStyleForElement(c, name) {
    ...
    case 'm':
      ...
      c.style.fontFamily = 'verdana,arial,sans-serif';
    ...
    case 'a':
      ...
      c.style.color = 'black';
      if (c.displaySpan) c.displaySpan.style.color = 'green';
      break;
    case 'b':
      c.style.backgroundColor = '#A5D200';
      c.style.color = 'black';
      c.style.cursor = 'pointer';
      if (c.displaySpan) c.displaySpan.style.color = 'white';
      break;
    case 'c':
      c.style.width = '500px';
  ...
  function Gb(i) {
    ...
    if (_w == "query") Ga += "font-size: 10px; text-align: right; color: green; padding-top: 3px; ";
  ...
  function D(name, gb) {
    if (Ka) {
      ...
      if (I.addRule) I.addRule(name, gb);
      else if (I.insertRule) I.insertRule(name + ' { ' + gb + ' }', I.cssRules.length);
    }
  }
  ...
  function call(Rb) {
    if (typeof $ != 'undefined') $.getScript(_url + '?q=' + encodeURIComponent(Rb));
    else {
      if (_xmlHttp && _xmlHttp.readyState != 0) _xmlHttp.abort();
      _xmlHttp = getXMLHTTP();
      if (_xmlHttp) {
        _xmlHttp.open('POST', _url, true);
        var parameter = '&q=' + encodeURIComponent(Rb);
        _xmlHttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        _xmlHttp.setRequestHeader('Content-length', parameter.length);
        _xmlHttp.setRequestHeader('Connection', 'close');
        _xmlHttp.onreadystatechange = function () {
          if (_xmlHttp.readyState == 4 && _xmlHttp.responseText) {
            if (_xmlHttp.responseText.charAt(0) == '<') _timeoutAdjustment--;
            else eval(_xmlHttp.responseText);
          }
        }
        _xmlHttp.send(parameter);
      }
    }
  }
...
jQuery.fn.myCounter = function (userOptions) {
  ...
  wo.myOptions = {
    ...
    initState: '0000:00:13:37'
  };
  $.extend(wo.myOptions, userOptions);
  ...
  wo.state = {
    ...
    s0maxed: function () {
      return (wo.state.v.s0 > 8);
    },
    s1maxed: function () {
      return (wo.state.v.s1 > 4);
    },
    m0maxed: function () {
      return (wo.state.v.m0 > 8);
    },
    m1maxed: function () {
      return (wo.state.v.m1 > 4);
    },
    h0maxed: function () {
      return ((wo.state.v.h0 > 8) || (wo.state.v.h1 > 1 && wo.state.v.h0 > 2));
    },
    h1maxed: function () {
      return (wo.state.v.h1 > 1);
    },
    d0maxed: function () {
      return (wo.state.v.d0 > 8);
    },
    d1maxed: function () {
      return (wo.state.v.d1 > 8);
    },
    d2maxed: function () {
      return (wo.state.v.d2 > 8);
    },
    d3maxed: function () {
      return (wo.state.v.d3 > 8);
    },
    inc: function () {
      if (!wo.state.s0maxed()) {
    ...
    if (start - target != 0) {
      ...
      if (opts.animate && opts.counterDelay > 5 * stepLength) {
        ...
        var t4 = setTimeout(function () {
          digitsList[digit].css('background-position', '+' + opts.digitWidth + 'px -' + target * opts.digitHeight + 'px');
        }, 3 * stepLength);
        var t3 = setTimeout(function () {
          digitsList[digit].css('background-position', '+' + 2 * opts.digitWidth + 'px -' + target * opts.digitHeight + 'px');
        }, 2 * stepLength);
        var t2 = setTimeout(function () {
          digitsList[digit].css('background-position', '-' + 2 * opts.digitWidth + 'px -' + start * opts.digitHeight + 'px');
        }, 1 * stepLength);
        var t1 = setTimeout(function () {
          digitsList[digit].css('background-position', '-' + opts.digitWidth + 'px -' + start * opts.digitHeight + 'px');
        }, 0 * stepLength);
      } else {
        digitsList[digit].css('background-position', '0 -' + target * opts.digitHeight + 'px');
      }
  ...
  $('head').append(styles);
  wo.append(container);
  initialize(wo.myOptions);
  interval = setInterval(function () {
    step()
  }, wo.myOptions.counterDelay);
};
